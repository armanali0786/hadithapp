...
export default function TagManagement() {
  ...
  const fetchItems = async () => {
    ...
    try {
      ...
      const data = res.data?.tags || res.data?.data || res.data || [];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await axios.put(`${API}/tags/${editing._id}`, form, headers);
      } else {
        await axios.post(`${API}/tags`, form, headers);
      }
      setShowModal(false);
      fetchItems();
    } catch (e) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/tags/${id}`, { headers: authHeaders() });
      fetchItems();
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="font-arabic text-isl-gold text-xl mb-0.5">الموضوعات</div>
          <h1 className="text-2xl font-bold text-gray-800">Topic Tags</h1>
          <p className="text-gray-500 text-sm mt-0.5">Manage topic tags for content categorization</p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <FiTag size={48} className="mx-auto text-gray-300 mb-3" />
            <p className="text-gray-500 font-medium">No tags yet</p>
            <p className="text-gray-400 text-sm">Click "Add Tag" to get started.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item, i) => (
                  <tr key={item._id} className={`hover:bg-gray-50 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{truncate(item.description, 40)}</td>
                    <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <h2 className="font-bold text-gray-800 text-lg">
                {editing ? 'Edit Tag' : 'Add Topic Tag'}
              </h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Faith, Prayer, Fasting"
                ...
                />
                <p className="text-xs text-gray-400 mt-1">Slug will be auto-generated by the server.</p>
              </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <input
                      ...
                      placeholder="#c9a227"
                      maxLength={7}
                      className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isl-green/30 focus:border-isl-green font-mono"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    min={0}
                    className="w-24 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isl-green/30 focus:border-isl-green"
                  />
                </div>
              </div>
              ...
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{ backgroundColor: form.color || '#c9a227' }}
                  />
                  <span
                    className="text-sm font-semibold px-3 py-1 rounded-full"
                    style={{
                      backgroundColor: form.color ? form.color + '22' : '#c9a22722',
