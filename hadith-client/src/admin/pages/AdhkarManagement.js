...
export default function AdhkarManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await axios.put(`${API}/adhkar/${editing._id}`, form, headers);
      } else {
        await axios.post(`${API}/adhkar`, form, headers);
      }
      setShowModal(false);
      fetchItems();
    } catch (e) {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Delete this dhikr?')) return;
    try {
      await axios.delete(`${API}/adhkar/${id}`, { headers: authHeaders() });
      fetchItems();
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="font-arabic text-isl-gold text-xl mb-0.5">الأذكار</div>
          <h1 className="text-2xl font-bold text-gray-800">Adhkar</h1>
          <p className="text-gray-500 text-sm mt-0.5">Manage morning and evening adhkar</p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <FiRepeat size={48} className="mx-auto text-gray-300 mb-3" />
            <p className="text-gray-500 font-medium">No adhkar yet</p>
            <p className="text-gray-400 text-sm">Click "Add Dhikr" to get started.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-isl-green text-white">
                <tr>
                  ...
                  <th className="text-left px-6 py-4 text-sm font-semibold">Count</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item, i) => (
                  <tr key={item._id} className={`hover:bg-gray-50 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${typeColors[item.type] || 'bg-gray-100 text-gray-600'}`}>
                        {item.type || '—'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-700">{item.count || 1}x</td>
                    <td className="px-6 py-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleChange}
                  placeholder="e.g. Abu Dawud 5068"
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-isl-green/30 focus:border-isl-green"
                />
              </div>
