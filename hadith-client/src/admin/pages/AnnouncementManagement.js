...
export default function AnnouncementManagement() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await axios.put(`${API}/announcements/${editing._id}`, fd, {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <div className="font-arabic text-isl-gold text-xl mb-0.5">الإعلانات</div>
          <h1 className="text-2xl font-bold text-gray-800">Announcements</h1>
          <p className="text-gray-500 text-sm mt-0.5">Manage platform announcements and notifications</p>
        </div>
        <button
          onClick={openAdd}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-gray-400 text-sm">Click "Add Announcement" to get started.</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-isl-green text-white">
                <tr>
                  ...
                  <th className="text-left px-6 py-4 text-sm font-semibold">Date Range</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {items.map((item, i) => (
                  <tr key={item._id} className={`hover:bg-gray-50 transition-colors ${i % 2 === 0 ? '' : 'bg-gray-50/50'}`}>
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize ${typeColors[item.type] || typeColors.general}`}>
                        {item.type || 'general'}
                      </span>
                    </td>
                    <td className="px-6 py-4">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">
                  Image {editing ? '(leave empty to keep existing)' : '(optional)'}
                </label>
                <input
                  type="file"
                  accept="image/*"
